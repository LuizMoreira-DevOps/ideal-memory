...
function calcular({
  ...
  const jogosNecessarios = Math.ceil((metaMensal + custosFixos) / lucroPorJogo);
  ...
  const receitaLiquidaEstimada = receitaBruta - custoVariavel - custosFixos;
...
document.getElementById("btn").addEventListener("click", () => {
  ...
  try {
    const params = {
      ...
      custosFixos: Number(document.getElementById("custosFixos").value || 0),
      custoPorJogo: Number(document.getElementById("custoPorJogo").value || 0),
      semanasNoMes: Number(
    ...
    if (
      ...
      throw new Error("Preencha pelo menos a Meta mensal e o Valor por jogo.");
    }
